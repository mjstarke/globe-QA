...
geostationary_satellite = ["GOES-15", "GOES-16", "METEOSAT-8", "METEOSAT-10", "METEOSAT-11", "HIMAWARI-8"][5]
...
obs = tools.filter_by_datetime(unfiltered_obs,
                               tools.get_cdf_datetime(cdf1, 0),
                               tools.get_cdf_datetime(cdf2, -1))

tools.patch_obs(obs, "geos_coincident_cat.csv", "tcc_geos_cat")

# Filter obs to those which have a valid GLOBE tcc, match a particular geostationary, and match either Aqua or Terra.
obs = [ob for ob in obs if
